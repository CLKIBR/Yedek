...
public class UpdateStudentProfileCommand : IRequest<UpdatedStudentProfileResponse>, ISecuredRequest, ICacheRemoverRequest, ILoggableRequest, ITransactionalRequest
{
    ...
    public string[] Roles => [Admin, Write, StudentProfilesOperationClaims.Update];

    public bool BypassCache { get; }
    public string? CacheKey { get; }
    public string[]? CacheGroupKey => ["GetStudentProfiles"];

    public class UpdateStudentProfileCommandHandler : IRequestHandler<UpdateStudentProfileCommand, UpdatedStudentProfileResponse>
    {
        private readonly IMapper _mapper;
        private readonly IStudentProfileRepository _studentProfileRepository;
        private readonly StudentProfileBusinessRules _studentProfileBusinessRules;

        public UpdateStudentProfileCommandHandler(IMapper mapper, IStudentProfileRepository studentProfileRepository,
        ...
        public async Task<UpdatedStudentProfileResponse> Handle(UpdateStudentProfileCommand request, CancellationToken cancellationToken)
        {
            ...
            studentProfile = _mapper.Map(request, studentProfile);

            await _studentProfileRepository.UpdateAsync(studentProfile!);

            UpdatedStudentProfileResponse response = _mapper.Map<UpdatedStudentProfileResponse>(studentProfile);
            return response;
        }
    }
}